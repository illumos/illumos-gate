 ...
 * TBL[2*i+1] = (double)(0.5/sqrtl(2) / sqrtl(*(double *) &
 *     (0x3ff0000000000000LL + (i << 46))));
 * TBL[128+2*i+0] = 1.0 / (*(double*)&(0x3ff0000000000000LL + (i << 46)));
 * TBL[128+2*i+1] = (double)(0.25 / sqrtl(*(double *) &
 *     (0x3ff0000000000000LL + (i << 46))));
 */
    ...
    int stridey, float *restrict pz, int stridez)
{
	...
	while (n > 1) {
		...
		if (n_n > 0)
			__vrhypotf_n(n_n, spx, stridex, spy, stridey, spz,
			    stridez);
	}
	if (n > 0) {
		ax0 = *(int *)px;
		...
		if (ax0 >= 0x7f800000 || ay0 >= 0x7f800000) {
			...
			*pz = 1.0f / 0.0f;
			...
			HI(&dbase0) =
			    (0x60000000 - ((ibase0 & 0x7fe00000) >> 1));
			...
			si0 = ibase0 & 0x7f0;
			xx0 = ((double *)((char *)
			    __vlibm_TBL_rhypotf + si0))[0];

			xx0 = (hyp0 - h_hi0) * xx0;
			res0 = ((double *)((char *)
			    __vlibm_TBL_rhypotf + si0))[1];
