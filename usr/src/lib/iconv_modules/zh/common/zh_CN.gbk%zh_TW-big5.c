...
typedef struct _icv_state {
	char keepc[2];	/* maximum # byte of GB chararor in two bytes area */
	short cstate;
		...
		switch (st->cstate) {
			...
			case C3:
				if ( gbk4_4th_byte((unsigned char)**inbuf) ) {
					/*
					 *  replace the four byte character with "__" in outbuf
					 *  due to that there hasn't corresponding code in BIG5
					 */
					if ( *outbytesleft < 2 ) {
						st->_errno = errno = E2BIG;
					} else {
...
/*
 *	gbk_to_big5: Convert gbk charactor to Big5.
