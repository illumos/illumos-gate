...
emlxs_pkt_thread(void *arg)
{
	...
	if (rval != FC_SUCCESS) {
		EMLXS_MSGF(EMLXS_CONTEXT, &emlxs_pkt_trans_msg,
		    "Deferred emlxs_pkt_send failed: status=%x pkt=%p",
		    rval, pkt);
...
emlxs_pkt_send(fc_packet_t *pkt, uint32_t now)
{
	...
	if (now) {
		...
		(void) thread_create(NULL, 0, emlxs_pkt_thread, (char *)pkt, 0,
    ...
    uint32_t datalen, int32_t sleep)
{
	...
	pkt->pkt_fca_private = (opaque_t)((uintptr_t)pkt +
	    sizeof (fc_packet_t));
...
#if (EMLXS_MODREV >= EMLXS_MODREV3)
	pkt_cookie = (emlxs_pkt_cookie_t *)((uintptr_t)pkt +
	...
	if (cmdlen) {
		...
		if (ddi_dma_mem_alloc(pkt->pkt_cmd_dma, cmdlen,
		    ...
		    (caddr_t *)&pkt->pkt_cmd, &real_len, &pkt->pkt_cmd_acc)
		    != DDI_SUCCESS) {
			(void) ddi_dma_free_handle(&pkt->pkt_cmd_dma);
		...
		if (ddi_dma_addr_bind_handle(pkt->pkt_cmd_dma, NULL,
		    ...
		    cb, NULL, pkt->pkt_cmd_cookie, &pkt->pkt_cmd_cookie_cnt)
		    != DDI_DMA_MAPPED)
#else
		if (ddi_dma_addr_bind_handle(pkt->pkt_cmd_dma, NULL,
		    pkt->pkt_cmd, real_len, DDI_DMA_WRITE | DDI_DMA_CONSISTENT,
		    cb, NULL, &pkt->pkt_cmd_cookie, &num_cookie)
		    != DDI_DMA_MAPPED)
#endif	/* >= EMLXS_MODREV3 */
		{
			(void) ddi_dma_mem_free(&pkt->pkt_cmd_acc);
	...
	if (rsplen) {
		...
		if (ddi_dma_mem_alloc(pkt->pkt_resp_dma, rsplen,
		    &emlxs_data_acc_attr, DDI_DMA_CONSISTENT, cb, NULL,
		    (caddr_t *)&pkt->pkt_resp, &real_len, &pkt->pkt_resp_acc)
		    != DDI_SUCCESS) {
			(void) ddi_dma_free_handle(&pkt->pkt_resp_dma);
		...
		if (ddi_dma_addr_bind_handle(pkt->pkt_resp_dma, NULL,
		    ...
		    cb, NULL, pkt->pkt_resp_cookie, &pkt->pkt_resp_cookie_cnt)
		    != DDI_DMA_MAPPED)
#else
		if (ddi_dma_addr_bind_handle(pkt->pkt_resp_dma, NULL,
		    pkt->pkt_resp, real_len, DDI_DMA_READ | DDI_DMA_CONSISTENT,
		    cb, NULL, &pkt->pkt_resp_cookie, &num_cookie)
		    != DDI_DMA_MAPPED)
#endif	/* >= EMLXS_MODREV3 */
		{
			(void) ddi_dma_mem_free(&pkt->pkt_resp_acc);
	...
	if (datalen) {
		...
		if (ddi_dma_mem_alloc(pkt->pkt_data_dma, datalen,
		    &emlxs_data_acc_attr, DDI_DMA_CONSISTENT, cb, NULL,
		    (caddr_t *)&pkt->pkt_data, &real_len, &pkt->pkt_data_acc)
		    != DDI_SUCCESS) {
			(void) ddi_dma_free_handle(&pkt->pkt_data_dma);
		...
		if (ddi_dma_addr_bind_handle(pkt->pkt_data_dma, NULL,
		    pkt->pkt_data, real_len, DDI_DMA_READ | DDI_DMA_WRITE |
		    DDI_DMA_CONSISTENT, cb, NULL, pkt->pkt_data_cookie,
