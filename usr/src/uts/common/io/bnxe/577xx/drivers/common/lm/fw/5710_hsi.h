...
struct xstorm_fcoe_extra_ag_context_section
{
	...
	u32_t __xfrqe_bd_addr_lo /* The Current transmission window in bytes */;
	u32_t __xfrqe_bd_addr_hi /* The current Send UNA sequence number */;
	u32_t __xfrqe_data1 /* The current local advertised window to FE. */;
#if defined(__BIG_ENDIAN)
...
#if defined(__BIG_ENDIAN)
	u16_t __xfrqe_mng /* Misc aggregated variable 6 */;
	u16_t __tcp_agg_vars7 /* Various aggregative variables*/;
#elif defined(__LITTLE_ENDIAN)
	u16_t __tcp_agg_vars7 /* Various aggregative variables*/;
	u16_t __xfrqe_mng /* Misc aggregated variable 6 */;
#endif
	u32_t __xfrqe_data0 /* aggregated value 10 */;
...
#endif
	u32_t agg_vars8;
		#define XSTORM_FCOE_AG_CONTEXT_AGG_MISC2                                             (0xFFFFFF<<0) /* BitField agg_vars8Various aggregative variables	Misc aggregated variable 2 */
		#define XSTORM_FCOE_AG_CONTEXT_AGG_MISC2_SHIFT                                       0
		#define XSTORM_FCOE_AG_CONTEXT_AGG_MISC3                                             (0xFF<<24) /* BitField agg_vars8Various aggregative variables	Misc aggregated variable 3 */
		#define XSTORM_FCOE_AG_CONTEXT_AGG_MISC3_SHIFT                                       24
#if defined(__BIG_ENDIAN)
	u16_t __cache_wqe_db /* Misc aggregated variable 0 */;
...
#endif
	u32_t agg_vars8;
		#define XSTORM_ISCSI_AG_CONTEXT_AGG_MISC2                                            (0xFFFFFF<<0) /* BitField agg_vars8Various aggregative variables	Misc aggregated variable 2 */
		#define XSTORM_ISCSI_AG_CONTEXT_AGG_MISC2_SHIFT                                      0
		#define XSTORM_ISCSI_AG_CONTEXT_AGG_MISC3                                            (0xFF<<24) /* BitField agg_vars8Various aggregative variables	Misc aggregated variable 3 */
...
enum classify_rule
{
	...
	CLASSIFY_RULE_OPCODE_VXLAN /* Add/remove a VXLAN entry (Inner MAC / VNI pair) */,
	MAX_CLASSIFY_RULE};
...
struct client_init_rx_data
{
	...
	u8_t dont_verify_rings_pause_thr_flg /* If set, the rings pause thresholds will not be verified by firmware. */;
	...
	u8_t handle_ptp_pkts_flg /* If set, this client handles PTP Packets */;
	u8_t reserved6[3];
	u32_t reserved7;
};
...
struct client_init_tx_data
{
	...
	u8_t force_default_pri_flg /* if set, force default priority */;
	u8_t tunnel_lso_inc_ip_id /* In case of LSO over IPv4 tunnel, whether to increment IP ID on external IP header or internal IP header */;
	u8_t refuse_outband_vlan_flg /* if set, the FW will not add outband vlan on packet (even if will exist on BD). */;
	u8_t tunnel_non_lso_pcsum_location /* In case of non-Lso encapsulated packets with L4 checksum offload, the pseudo checksum location - on packet or on BD. */;
	u8_t tunnel_non_lso_outer_ip_csum_location /* In case of non-Lso encapsulated packets with outer L3 ip checksum offload, the pseudo checksum location - on packet or on BD. */;
};
...
struct client_update_ramrod_data
{
	...
	u8_t refuse_outband_vlan_flg /* If set, the FW will not add outband vlan on packet (even if will exist on BD). */;
	u8_t refuse_outband_vlan_change_flg /* If set, refuse_outband_vlan_flg will be updated. */;
	u8_t tx_switching_flg /* If set, tx switching will be done to packets on this connection. */;
	u8_t tx_switching_change_flg /* If set, tx_switching_flg will be updated. */;
	u8_t handle_ptp_pkts_flg /* If set, this client handles PTP Packets */;
	u8_t handle_ptp_pkts_change_flg /* If set, handle_ptp_pkts_flg will be updated. */;
	u16_t reserved1;
...
struct eth_classify_cmd_header
{
	u8_t cmd_general_data;
		...
		#define ETH_CLASSIFY_CMD_HEADER_OPCODE                                               (0x3<<2) /* BitField cmd_general_data	command opcode for MAC/VLAN/PAIR/VXLAN (use enum classify_rule) */
...
/*
 * Command for adding/removing a VXLAN classification rule $$KEEP_ENDIANNESS$$
 */
...
struct eth_fast_path_rx_cqe
{
	...
	u16_t pkt_len_or_gro_seg_len /* Packet length (for non-TPA CQE) or GRO Segment Length (for TPA in GRO Mode) otherwise 0 */;
...
{
#if defined(__BIG_ENDIAN)
	u16_t dst_mid /* destination mac address 16 middle bits */;
	u16_t dst_lo /* destination mac address 16 low bits */;
#elif defined(__LITTLE_ENDIAN)
	u16_t dst_lo /* destination mac address 16 low bits */;
	u16_t dst_mid /* destination mac address 16 middle bits */;
#endif
#if defined(__BIG_ENDIAN)
	u16_t src_lo /* source mac address 16 low bits */;
	u16_t dst_hi /* destination mac address 16 high bits */;
#elif defined(__LITTLE_ENDIAN)
	u16_t dst_hi /* destination mac address 16 high bits */;
	u16_t src_lo /* source mac address 16 low bits */;
#endif
#if defined(__BIG_ENDIAN)
	u16_t src_hi /* source mac address 16 high bits */;
	u16_t src_mid /* source mac address 16 middle bits */;
#elif defined(__LITTLE_ENDIAN)
	u16_t src_mid /* source mac address 16 middle bits */;
	u16_t src_hi /* source mac address 16 high bits */;
#endif
...
struct eth_tunnel_data
{
...
#endif
#if defined(__BIG_ENDIAN)
	u16_t fw_ip_hdr_csum /* Fw Ip header checksum (with ALL ip header fields) for the outer IP header */;
	u16_t dst_hi /* destination mac address 16 high bits */;
#elif defined(__LITTLE_ENDIAN)
	u16_t dst_hi /* destination mac address 16 high bits */;
	u16_t fw_ip_hdr_csum /* Fw Ip header checksum (with ALL ip header fields) for the outer IP header */;
	...
	u8_t ip_hdr_start_inner_w /* Inner IP header offset in WORDs (16-bit) from start of packet */;
	u16_t pseudo_csum /* Pseudo checksum with  length  field=0 */;
#elif defined(__LITTLE_ENDIAN)
	u16_t pseudo_csum /* Pseudo checksum with  length  field=0 */;
	u8_t ip_hdr_start_inner_w /* Inner IP header offset in WORDs (16-bit) from start of packet */;
	u8_t flags;
...
{
	u8_t rss_engine_id;
	u8_t rss_mode /* The RSS mode for this function */;
	u16_t capabilities;
		...
		#define ETH_RSS_UPDATE_RAMROD_DATA_EN_5_TUPLE_CAPABILITY                             (0x1<<8) /* BitField capabilitiesFunction RSS capabilities	configuration of the 5-tuple capability */
		#define ETH_RSS_UPDATE_RAMROD_DATA_EN_5_TUPLE_CAPABILITY_SHIFT                       8
		#define ETH_RSS_UPDATE_RAMROD_DATA_NVGRE_KEY_ENTROPY_CAPABILITY                      (0x1<<9) /* BitField capabilitiesFunction RSS capabilities	configuration of NVGRE key entropy capability. Requires gre_inner_hdrs_capability is not set. */
		#define ETH_RSS_UPDATE_RAMROD_DATA_NVGRE_KEY_ENTROPY_CAPABILITY_SHIFT                9
		#define ETH_RSS_UPDATE_RAMROD_DATA_GRE_INNER_HDRS_CAPABILITY                         (0x1<<10) /* BitField capabilitiesFunction RSS capabilities	configuration of GRE Inner Headers capability. Requires nvgre_key_entropy_capability is not set. */
		#define ETH_RSS_UPDATE_RAMROD_DATA_GRE_INNER_HDRS_CAPABILITY_SHIFT                   10
		#define ETH_RSS_UPDATE_RAMROD_DATA_UPDATE_RSS_KEY                                    (0x1<<11) /* BitField capabilitiesFunction RSS capabilities	if set update the rss keys */
		#define ETH_RSS_UPDATE_RAMROD_DATA_UPDATE_RSS_KEY_SHIFT                              11
		#define ETH_RSS_UPDATE_RAMROD_DATA_RESERVED                                          (0xF<<12) /* BitField capabilitiesFunction RSS capabilities	 */
...
struct eth_tx_start_bd
{
	...
	u16_t nbytes /* Size of the data represented by the BD */;
	u16_t vlan_or_ethertype /* Vlan structure: vlan_id is in lsb, then cfi and then priority vlan_id 12 bits (lsb), cfi 1 bit, priority 3 bits. In E2, this field should be set with etherType for VFs with no vlan */;
...
struct eth_tx_parse_2nd_bd
{
	...
	u8_t reserved4;
	u8_t tunnel_udp_hdr_start_w /* Offset (in WORDs) from start of packet to tunnel UDP header. (if exist) */;
	u8_t fw_ip_hdr_to_payload_w /* In IpV4, the length (in WORDs) from the FW IpV4 header start to the payload start. In IpV6, the length (in WORDs) from the FW IpV6 header end to the payload start. However, if extension headers are included, their length is counted here as well. */;
	u16_t fw_ip_csum_wo_len_flags_frag /* For the IP header which is set by the FW, the IP checksum without length, flags and fragment offset. */;
	u16_t hw_ip_id /* The IP ID to be set by HW for LSO packets in tunnel mode. */;
	u32_t tcp_send_seq /* The TCP sequence number for LSO packets. */;
};
...
{
	u8_t update_ipv4 /* none, enable or disable */;
	u8_t update_ipv6 /* none, enable or disable */;
	u8_t client_id /* client init flow control data */;
	u8_t max_tpa_queues /* maximal TPA queues allowed for this client */;
	u8_t max_sges_for_packet /* The maximal number of SGEs that can be used for one packet. depends on MTU and SGE size. must be 0 if SGEs are disabled */;
	u8_t complete_on_both_clients /* If set and the client has different sp_client, completion will be sent to both rings */;
	u8_t dont_verify_rings_pause_thr_flg /* If set, the rings pause thresholds will not be verified by firmware. */;
	u8_t tpa_mode /* TPA mode to use (LRO or GRO) */;
	u16_t sge_buff_size /* Size of the buffers pointed by SGEs */;
	u16_t max_agg_size /* maximal size for the aggregated TPA packets, reprted by the host */;
...
{
	u32_t fcoe_rx_pkt_cnt /* Number of FCoE packets that were legally received */;
	u32_t fcoe_rx_byte_cnt /* Number of FCoE bytes that were legally received */;
};
...
{
	u32_t fcoe_ver_cnt /* Number of packets with wrong FCoE version */;
	u32_t fcoe_rx_drop_pkt_cnt /* Number of FCoE packets that were dropped */;
};
...
{
	u32_t fc_crc_cnt /* Number of packets with FC CRC error */;
	u32_t eofa_del_cnt /* Number of packets with EOFa delimiter */;
	u32_t miss_frame_cnt /* Number of missing packets */;
	u32_t seq_timeout_cnt /* Number of sequence timeout expirations (E_D_TOV) */;
	u32_t drop_seq_cnt /* Number of Sequences that were sropped */;
	u32_t fcoe_rx_drop_pkt_cnt /* Number of FCoE packets that were dropped */;
	u32_t fcp_rx_pkt_cnt /* Number of FCP packets that were legally received */;
	u32_t reserved0;
};
...
{
	u32_t fcoe_tx_pkt_cnt /* Number of transmitted FCoE packets */;
	u32_t fcoe_tx_byte_cnt /* Number of transmitted FCoE bytes */;
	u32_t fcp_tx_pkt_cnt /* Number of transmitted FCP packets */;
	u32_t reserved0;
};
...
{
	u8_t vlan_conf;
		#define XSTORM_FCOE_VLAN_CONF_PRIORITY                                               (0x7<<0) /* BitField vlan_conf	Original priority */
		...
		#define XSTORM_FCOE_VLAN_CONF_RESERVED                                               (0xF<<4) /* BitField vlan_conf	 */
...
#endif
#if defined(__BIG_ENDIAN)
	u16_t reserved2 /* The ID of the statistics client for counting common/L2 statistics */;
	u8_t statistics_counter_id /* The ID of the statistics client for counting common/L2 statistics */;
...
{
	u8_t afex_vif_list_command /* set get, clear all a VIF list id defined by enum vif_list_rule_kind */;
...
{
	u32_t port_rate;
	u16_t vnic_min_rate[4] /* rates are in Mbps */;
	u16_t vnic_max_rate[4] /* rates are in Mbps */;
	u16_t cos_min_rate[MAX_COS_NUMBER] /* rates are in Mbps */;
...
{
	u8_t vf_id /* VF ID (0-63) */;
	u8_t err_id /* reason for malicious notification */;
	u16_t reserved1;
	u32_t reserved2;
	u32_t reserved3;
};
...
{
	u8_t function_mode /* the function mode */;
	u8_t allow_npar_tx_switching /* If set, inter-pf tx switching is allowed in Switch Independant function mode. (E2/E3 Only) */;
	...
	u8_t path_id;
	u8_t network_cos_mode /* The cos mode for network traffic. */;
	u8_t dmae_cmd_id /* The DMAE command id to use for FW DMAE transactions */;
	u8_t tunnel_mode /* Tunnel Mode to enable on the Function (E2/E3 Only) */;
	u8_t gre_tunnel_type /* GRE Tunnel Type to enable on the Function (Only for tunnel_mode is TUNNEL_GRE) */;
	u8_t tunn_clss_en /* If set, tunneled packets are classified according to their inner headers (Only for tunnel_mode is TUNNEL_VXLAN or tunnel_mode is TUNNEL_GRE and gre_tunnel_type is NVGRE_TUNNEL) */;
	u8_t inner_gre_rss_en /* If set, RSS on the inner headers of GRE tunneled packets is enabled */;
	u8_t sd_accept_mf_clss_fail /* If set, accept packets that fail Multi-Function Switch-Dependent classification. Only one VNIC on the port can have this set to 1 */;
	u16_t vxlan_dst_port /* UDP Destination Port to be recognised as VXLAN tunneled packets */;
	u16_t sd_accept_mf_clss_fail_ethtype /* Ethertype to match in the case of sd_accept_mf_clss_fail_match_ethtype */;
	u16_t sd_vlan_eth_type /* Value of ether-type to use in the case of switch dependant multi-function mode. Setting this to 0 uses the default value of 0x8100 */;
	u8_t sd_vlan_force_pri_flg /* If set, the SD Vlan Priority is forced to the value of the sd_vlan_pri_force_val field regardless of the DCB or inband VLAN priority. */;
	u8_t sd_vlan_force_pri_val /* value to force SD Vlan Priority if sd_vlan_pri_force_flg is set */;
	u8_t sd_accept_mf_clss_fail_match_ethtype /* If set, accepted packets must match the ethertype of sd_clss_fail_ethtype */;
	u8_t no_added_tags /* If set, the mfTag length is always zero (used in UFP) */;
};
...
{
	u8_t vif_id_change_flg /* If set, vif_id will be checked */;
	u8_t afex_default_vlan_change_flg /* If set, afex_default_vlan will be checked */;
	u8_t allowed_priorities_change_flg /* If set, allowed_priorities will be checked */;
	u8_t network_cos_mode_change_flg /* If set, network_cos_mode will be checked */;
	u16_t vif_id /* value of VIF id in case of NIV multi-function mode */;
	u16_t afex_default_vlan /* value of default Vlan in case of NIV mf */;
	u8_t allowed_priorities /* bit vector of allowed Vlan priorities for this VIF */;
	u8_t network_cos_mode /* The cos mode for network traffic. */;
	u8_t lb_mode_en_change_flg /* If set, lb_mode_en will be checked */;
	u8_t lb_mode_en /* If set, niv loopback mode will be enabled */;
	u8_t tx_switch_suspend_change_flg /* If set, tx_switch_suspend will be checked */;
	u8_t tx_switch_suspend /* If set, TX switching TO this function will be disabled and packets will be dropped */;
	u8_t echo;
	u8_t update_tunn_cfg_flg /* If set, tunneling config for the function will be updated according to the following fields */;
	u8_t tunnel_mode /* Tunnel Mode to enable on the Function (E2/E3 Only) */;
	u8_t gre_tunnel_type /* GRE Tunnel Type to enable on the Function (Only for tunnel_mode is TUNNEL_GRE) */;
	u8_t tunn_clss_en /* If set, tunneled packets are classified according to their inner headers (Only for tunnel_mode is TUNNEL_VXLAN or tunnel_mode is TUNNEL_GRE and gre_tunnel_type is NVGRE_TUNNEL) */;
	u8_t inner_gre_rss_en /* If set, RSS on the inner headers of GRE tunneled packets is enabled */;
	u16_t vxlan_dst_port /* UDP Destination Port to be recognised as VXLAN tunneled packets */;
	u8_t sd_vlan_force_pri_change_flg /* If set, the SD VLAN Priority Fixed configuration is updated from fields sd_vlan_pri_force_flg and sd_vlan_pri_force_val */;
	u8_t sd_vlan_force_pri_flg /* If set, the SD Vlan Priority is forced to the value of the sd_vlan_pri_force_val field regardless of the DCB or inband VLAN priority. */;
	u8_t sd_vlan_force_pri_val /* value to force SD Vlan Priority if sd_vlan_pri_force_flg is set */;
	u8_t sd_vlan_tag_change_flg /* If set, the SD VLAN Tag is changed according to the field sd_vlan_tag */;
	u8_t sd_vlan_eth_type_change_flg /* If set, the SD VLAN Ethertype is changed according to the field sd_vlan_eth_type */;
	u8_t reserved1;
	u16_t sd_vlan_tag /* New value of Outer Vlan in case of switch depended multi-function mode */;
	u16_t sd_vlan_eth_type /* New value of ether-type in the case of switch dependant multi-function mode. Setting this to 0 restores the default value of 0x8100 */;
};
...
struct hc_sp_status_block_data
{
	...
	u8_t rsrv1;
	u8_t state;
	u8_t igu_seg_id /* segment id of the IGU */;
	u8_t igu_sb_id /* sb_id within the IGU */;
#elif defined(__LITTLE_ENDIAN)
	u8_t igu_sb_id /* sb_id within the IGU */;
	u8_t igu_seg_id /* segment id of the IGU */;
...
struct tstorm_per_port_stats
{
	...
	u32_t packet_drop /* general packet drop conter- incremented for every packet drop */;
	u32_t reserved;
};
...
{
	u8_t drift_adjust_cmd /* Timesync Drift Adjust Command */;
	u8_t offset_cmd /* Timesync Offset Command */;
	u8_t add_sub_drift_adjust_value /* Whether to add(1)/subtract(0) Drift Adjust Value from the Offset */;
	u8_t drift_adjust_value /* Drift Adjust Value (in ns) */;
	u32_t drift_adjust_period /* Drift Adjust Period (in us) */;
...
enum tunnel_mode
{
	TUNN_MODE_NONE /* No tunnel */,
	TUNN_MODE_VXLAN /* VXLAN tunnel */,
	TUNN_MODE_GRE /* GRE tunnel */,
	MAX_TUNNEL_MODE};
...
#if defined(__BIG_ENDIAN)
	u8_t sleep /* Measure including a thread sleep */;
	u8_t enable /* Enable PCI Latency measurements */;
	u8_t func_id /* Function ID */;
	u8_t read_size /* Amount of bytes to read */;
#elif defined(__LITTLE_ENDIAN)
	u8_t read_size /* Amount of bytes to read */;
	u8_t func_id /* Function ID */;
	u8_t enable /* Enable PCI Latency measurements */;
...
#endif
#if defined(__BIG_ENDIAN)
	u16_t num_meas /* Number of measurements to make */;
	u8_t reserved;
	u8_t period_10us /* Number of 10s of microseconds to wait between measurements */;
#elif defined(__LITTLE_ENDIAN)
	u8_t period_10us /* Number of 10s of microseconds to wait between measurements */;
	u8_t reserved;
	u16_t num_meas /* Number of measurements to make */;
...
{
#if defined(__BIG_ENDIAN)
	u16_t max_time_ns /* Maximum Time for a read (in ns) */;
	u16_t min_time_ns /* Minimum Time for a read (in ns) */;
#elif defined(__LITTLE_ENDIAN)
	u16_t min_time_ns /* Minimum Time for a read (in ns) */;
	u16_t max_time_ns /* Maximum Time for a read (in ns) */;
#endif
#if defined(__BIG_ENDIAN)
	u16_t reserved;
	u16_t num_reads /* Number of reads - Used for Average */;
#elif defined(__LITTLE_ENDIAN)
	u16_t num_reads /* Number of reads - Used for Average */;
	u16_t reserved;
#endif
