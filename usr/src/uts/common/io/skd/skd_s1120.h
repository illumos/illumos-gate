...
struct fit_msg_hdr {
	...
	uint64_t _reserved2[7];	/* Forces alignment. */
};
...
struct driver_inquiry_data {
	...
	uint8_t pageCode;
	uint16_t pageLength;
	uint16_t pcieBusNumber;
	uint8_t pcieDeviceNumber;
	uint8_t pcieFunctionNumber;
	uint8_t pcieLinkSpeed;
	uint8_t pcieLinkLanes;
	uint16_t pcieVendorId;
	uint16_t pcieDeviceId;
	uint16_t pcieSubsystemVendorId;
	uint16_t pcieSubsystemDeviceId;
	uint8_t reserved1[2];
	uint8_t reserved2[3];
	uint8_t driverVersionLength;
	uint8_t driverVersion[0x14];
};
