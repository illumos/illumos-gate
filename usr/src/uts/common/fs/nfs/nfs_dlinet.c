...
int		dl_attach(ldi_handle_t, int);
int		dl_bind(ldi_handle_t, uint32_t, uint32_t, uint32_t,
			    uint32_t);
int		dl_phys_addr(ldi_handle_t, struct ether_addr *);
...
dl_attach(ldi_handle_t lh, int unit)
{
	...
	(void) ldi_putmsg(lh, mp);
	if ((error = ldi_getmsg(lh, &mp, (timestruc_t *)NULL)) != 0) {
		nfs_perror(error, "dl_attach: ldi_getmsg failed: %m\n");
		return (error);
	}
	...
	uint_t conn_mgmt)
{
	...
	(void) ldi_putmsg(lh, mp);
	if ((error = ldi_getmsg(lh, &mp, (timestruc_t *)NULL)) != 0) {
		nfs_perror(error, "dl_bind: ldi_getmsg failed: %m\n");
		return (error);
	}
...
dl_phys_addr(ldi_handle_t lh, struct ether_addr *eaddr)
{
	...
	if (error == ETIME) {
		...
		nfs_perror(error, "dl_phys_addr: ldi_getmsg failed: %m\n");
		return (error);
	}
