...
rdip(struct inode *ip, struct uio *uio, int ioflag, cred_t *cr)
{
	...
	/*
	 * If cache_read_ahead is enabled, we will
	 * release the pages at the tail of the cache
	 * list, otherwise we will put them at the head.
	 */
	if (cache_read_ahead)
		cachemode = SM_FREE | SM_ASYNC;
	else
		cachemode = SM_FREE | SM_DONTNEED | SM_ASYNC;
