...
typedef struct menlo_fw_download_cmd {
	...
	uint32_t length;	/* Firmware image length in bytes (4 byte */
				/* aligned) */
...
typedef struct menlo_get_config_rsp {
	...
	uint32_t log_cfg_size;	/* Size of log config region.  Needed for */
				/* MENLO_CMD_GET_LOG_CONFIG */
	uint32_t panic_log_size;	/* Size of panic log region.  Needed */
	...
	uint32_t fcoe_format;	/* Bit field - single bit will be set   (See */
				/* below) */
...
typedef struct menlo_log_config_rsp {
	...
	uint32_t data;	/* First word of array:  menlo_log_t log[num_logs]  */
...
typedef struct menlo_log_data_rsp {
...
#ifdef EMLXS_BIG_ENDIAN
	uint16_t rsvd;
	uint16_t head;	/* Index of oldest log entry in circular data array */
#endif	/* EMLXS_BIG_ENDIAN */
	...
	uint32_t data;	/* char array[menlo_log_t.num_entries] */
...
#define	MENLO_PANIC_TYPE_EXCEPTION		0xdead0002
	...
	uint16_t head;	/* Index of oldest log entry in circular data buffer */
...
typedef struct menlo_diag_log_data_rsp {
	uint32_t code;	/* Response code */
	uint32_t data_length;	/* Length of the diagnostic log buffer */
				/* (bytes) */
	uint32_t data;	/* menlo_diag_log_t log of size data_length bytes */
...
#define	DIAG_TEST_STATUS_ABORT		0xD0000003
	...
	uint32_t data;	/* menlo_diag_log_entry_t entry[num_tests] */

} menlo_diag_log_t;

typedef struct menlo_diag_log_entry {
	uint32_t status;	/* Test status See DIAG_TEST_STATUS_XXXXX */
				/* above */
	uint32_t data;	/* uint32_t array[menlo_diag_log_t.status_length] */
...
typedef struct menlo_image_hdr {
	...
	uint32_t image_length;	/* length of the image without padding */
	uint32_t rsvd3;
	uint32_t rsvd4;
...
#define	MENLO_IMAGE_TYPE_DIAGNOSTICS 	2
	...
	uint32_t checksum;	/* 32bit XOR checksum -- needs to be at the */
				/* end */
