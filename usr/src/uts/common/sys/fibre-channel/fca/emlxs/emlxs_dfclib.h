...
#define	BUFF_TYPE_32BIT		0x10	/* """",1=32 bit addr 0=64 bit */
					/* addr */
#define	BUFF_TYPE_SPECIA	0x20	/* Not Implemented with LP6000 */
...
typedef struct dfc_cfgparam {
	...
	uint32_t a_current;
	uint16_t a_flag;
#define	CFG_EXPORT		0x1	/* Export this parameter to the end */
					/* user */
...
#define	NODE_LIMBO		0x1	/* entry to hang around for wwpn/sid */
#define	NODE_LOGOUT		0x2	/* NL_PORT not logged in entry cached */
#define	NODE_PLOGI		0x3	/* PLOGI was sent to NL_PORT */
#define	NODE_LOGIN		0x4	/* NL_PORT is logged in / REG_LOGINed */
#define	NODE_PRLI		0x5	/* PRLI was sent to NL_PORT */
#define	NODE_ALLOC		0x6	/* NL_PORT ready to start adapter I/O */
...
#define	DFC_DRVINFO_LEADVILLE	0x14	/* Solaris ULP */

	uint16_t reserved;
	uint8_t rmLevel;	/* Remote Management (HBAnyware) Support */
				/* Level */
...
#define	FC_REG_LINK_EVENT	0x01	/* Register for link up / down events */
#define	FC_REG_RSCN_EVENT	0x02	/* Register for RSCN events */
#define	FC_REG_CT_EVENT		0x04	/* Register for CT request events */
#define	FC_REG_MULTIPULSE_EVENT	0x08	/* Register for MultiPulse events */
#define	FC_REG_DUMP_EVENT	0x10	/* Register for Diagnostic Dump */
					/* events */
#define	FC_REG_TEMP_EVENT	0x20	/* Register for Temperature events */
...
#define	DFC_BIND_RPTLUNST	0x40	/* Node is in report lun cmpl state */
...
#define	CT_OP_RSAT		0x0141	/* ReadSA Table */
#define	CT_OP_WSAT		0x0142	/* Write SA Table */
#define	CT_OP_RSAH		0x0143	/* ReadSA Table Header */
#define	CT_OP_WSAH		0x0144	/* Write SA Table Header */
#define	CT_OP_RACT		0x0145	/* ReadAccess Control Table */
#define	CT_OP_WACT		0x0146	/* Write Access Control Table */
#define	CT_OP_RKT		0x0147	/* ReadKey Table Table */
#define	CT_OP_WKT		0x0148	/* Write Key Table Table */
#define	CT_OP_SSC		0x0149	/* Cause SA Table */
					/* re-read;sync */
...
#define	CT_OP_GRV		0x0179	/* Get Range Value for Bus#, Target#, */
					/* Lun# */
...
typedef struct dfc_vport_attrs {
	uint8_t version;	/* 2 = version of this structure, for */
				/* compatibility check */
	uint8_t reserved1[3];

	uint8_t wwpn[8];	/* virtual port WWPN */
	uint8_t wwnn[8];	/* virtual port WWNN */
	char name[256];		/* name to be register with the fabric */

	uint32_t options;

	uint32_t portFcId;	/* D-ID; set when the N-port is created */
				/* successfully */
...
typedef struct dfc_vport_entry {
	...
	uint32_t PortFcId;	/* FC port ID assigned to this virtual port */
...
typedef struct dfc_vport_get_nodeinfo {
	...
	/* start of the dfc_vport_nodeinfo_entry_t array */
...
#define	DFC_AUTH_TYPE_DHCHAP		1	/* Only one supported */
