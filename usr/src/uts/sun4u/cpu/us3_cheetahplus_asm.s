...
#define	PN_L2_REFLUSH_LINE(l2_index, scr2, scr3)			\
	set	PN_L2_MAX_SET, scr2;					\
	set	PN_L2_SET_SIZE, scr3;					\
1:									\
	ldxa	[l2_index + scr2]ASI_L2_TAG, %g0;			\
...
/*
 * Panther version of L2_flush_line. Flushes the line corresponding
 * to physaddr from the L2 cache
 *
 * physaddr	Input: Physical address to flush.
 *              Output: Physical address to flush (preserved).
 * l2_idx_out	Input: scratch register.
 ...
 *
 */
#define	PN_L2_FLUSH_LINE(physaddr, l2_idx_out, scr2, scr3)	\
	set	PN_L2_SET_SIZE, l2_idx_out;		\
	sub	l2_idx_out, 1, l2_idx_out;		\
	and	physaddr, l2_idx_out, l2_idx_out;	\
	set	PN_L2_IDX_DISP_FLUSH, scr3;		\
	or	l2_idx_out, scr3, l2_idx_out;		\
	PN_L2_REFLUSH_LINE(l2_idx_out, scr2, scr3)
...
/*ARGSUSED*/
void
casxa_physical_addr(uint64_t start_addr, uint64_t iteration_count)
{}

#else
	ENTRY(casxa_physical_addr)
1:
	casxa	[%o0]ASI_MEM, %g0, %g0
	brnz,pt	%o1, 1b
	 dec	%o1
...
/*ARGSUSED*/
void
rw_physical_addr(uint64_t start_addr, uint64_t end_addr)
{}

#else
	ENTRY(rw_physical_addr)
	ldxa	[%o0]ASI_MEM, %g1
1:
	stxa	%g1, [%o0]ASI_MEM
	add	%o0, 8, %o0
...
/*ARGSUSED*/
void
read_from_physical_addr(uint64_t start_addr, uint64_t count, uint64_t buffer)
{}

#else
	ENTRY(read_from_physical_addr)
	clr	%o4
	ba,a	2f
1:
	stx	%g1, [%o2]
...
2:
	...
	retl
	nop
	SET_SIZE(read_from_physical_addr)
...
/*ARGSUSED*/
int
unretire_l2_alternate(uint64_t tag_addr)
{return 0;}
...
/*ARGSUSED*/
int
unretire_l3_alternate(uint64_t tag_addr)
{return 0;}
