...
nvlist_t *
fmd_protocol_suspects(nvlist_t *de_fmri,
    const char *uuid, const char *code, uint_t argc, nvlist_t **argv, int domsg)
{
	struct timeval tv;
	int64_t tod[2];
	nvlist_t *nvl;
	...
	if (!domsg) {
		err |= nvlist_add_boolean_value(nvl, FM_SUSPECT_MESSAGE,
		    B_FALSE);
	}
...
nvlist_t *
fmd_protocol_resource(const char *class, nvlist_t *fmri, const char *uuid,
