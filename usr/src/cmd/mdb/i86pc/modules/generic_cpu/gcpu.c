...
gcpu_mpt_dump(uintptr_t addr, uint_t flags, int argc, const mdb_arg_t *argv)
{
	static const char *const whatstrs[] = {
		"cyc-err",	/* GCPU_MPT_WHAT_CYC_ERR */
		"poke-err",	/* GCPU_MPT_WHAT_POKE_ERR */
		"unfault",	/* GCPU_MPT_WHAT_UNFAULTING */
	};

	gcpu_mca_poll_trace_t mpt;
	const char *what;
	...
	if (mdb_vread(&mpt, sizeof (mpt), addr) != sizeof (mpt)) {
		mdb_warn("failed to read gcpu_mca_poll_trace_t at 0x%p", addr);
		return (DCMD_ERR);
	}
...
typedef struct mptwalk_data {
	...
	gcpu_mca_poll_trace_t *mw_trace;
	size_t mw_tracesz;
	uint_t mw_tracenent;
	uint_t mw_curtrace;
} mptwalk_data_t;
...
gcpu_mptwalk_init(mdb_walk_state_t *wsp)
{
	...
	if (mdb_lookup_by_name("gcpu_mca_poll_trace_nent", &sym) < 0 ||
	    sym.st_size != sizeof (uint_t) || mdb_vread(&nent, sizeof (uint_t),
	    sym.st_value) != sizeof (uint_t)) {
		mdb_warn("failed to read gcpu_mca_poll_trace_nent from kernel");
		return (WALK_ERR);
	}
	...
	mw->mw_tracesz = nent * sizeof (gcpu_mca_poll_trace_t);
...
gcpu_mptwalk_step(mdb_walk_state_t *wsp)
{
	...
	gcpu_mca_poll_trace_t *thismpt, *prevmpt;
	int prev, rv;

	thismpt = &mw->mw_trace[mw->mw_curtrace];

	rv = wsp->walk_callback(mw->mw_traceaddr + (mw->mw_curtrace *
	    sizeof (gcpu_mca_poll_trace_t)), thismpt, wsp->walk_cbdata);
