...
#define TEXT_DOMAIN "SYS_TEST"
	...
	if (   (   *cp1 == 'D'
	    ...
	    if (nsyms >= MAXSYMS) {
		prname ();
		fprintf (stderr, gettext("too many symbols.\n"));
		exit (2);
	    }
 ...
 unrec:
	    prname ();
	    fprintf (stderr, gettext("unrecognized option: %s\n"), cp);
	    goto usage;
	}
    }
    if (nsyms == 0) {
...
Usage: %s [-l] [-t] [-c] [[-Dsym] [-Usym] [-idsym] [-iusym]]... [file]\n\
    ...
    if (argc > 1) {
	prname ();
	fprintf (stderr, gettext("can only do one file.\n"));
    }
    else if (argc == 1) {
	filename = *curarg;
...
error (err, line, depth)
{
...
#ifndef TESTING
    fprintf (stderr, gettext("Error in %s line %d: %s.\n"),
    ...
    fprintf (stderr, gettext("ifdef depth: %d\n"), depth);
#endif
