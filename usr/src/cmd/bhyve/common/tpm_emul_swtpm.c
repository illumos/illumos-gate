    ...
    uint32_t rsp_size)
{
	...
	while (cmd_size > 0) {
		...
		if (len == -1) {
			if (errno == EINTR)
				continue;
			err(1, "%s: cmd send failed, is swtpm running?",
			    __func__);
		}
		cmd += len;
		cmd_size -= len;
	...
	size_t buflen = rsp_size;
	size_t rcvd = 0;
	while (buflen > 0 && rcvd < sizeof (struct tpm_resp_hdr)) {
		len = recv(tpm->fd, rsp, buflen, 0);
		if (len == -1) {
			if (errno == EINTR)
				continue;
			err(1, "%s: rsp recv failed, is swtpm running?",
			    __func__);
		}
		if (len == 0)
			break;
