...
main(void)
{
	...
	/* Reference start_sector or linker will elliminate it. */
	if (start_sector != 0)
		printf("isoboot: starting...\n");
	...
	*cmd = '\0';
	...
	for (;;) {
		if (!OPT_CHECK(RBX_QUIET)) {
			printf("\nillumos/x86 boot\n");
			if (dsk.part == -1) {
				printf("Default: %u:%s(%u)%s\n",
				    dsk.drive & DRV_MASK, dev_nm[dsk.type],
				    dsk.unit, kname);
			} else {
				printf("Default: %u:%s(%up%u)%s\n",
				    dsk.drive & DRV_MASK, dev_nm[dsk.type],
...
load(void)
{
	...
	if (!(ino = cd9660_lookup(kname))) {
		if (!ls) {
			printf("%s: No %s on %u:%s(%u", BOOTPROG,
