...
mkprog(struct sym_test *st)
{
	...
	/*
	 * For func probes with a return value, emit the RESULT() macro that
	 * selects brace-initialisation (C++11+) or plain assignment (C++98).
	 */
	bool has_rtype = (st->st_type == SYM_FUNC &&
	    st->st_rtype != NULL &&
	    strcmp(st->st_rtype, "") != 0 &&
	...
	case SYM_FUNC:
		...
		/*
		 * Use the RESULT() macro for non-void return values; it
		 * expands to brace-initialisation in C++11 and later (to
		 * catch narrowing/ambiguity errors), and assignment in C++98.
...
find_cxx_includes(void)
{
	...
	common_flags = (const char *) newflags;
